...
pub fn ExportButton(#[prop(optional)] color: Option<String>, #[prop(optional)] contrast:Option<f64>, #[prop(optional)] class: Option<String>) -> impl IntoView {
    ...
    view! {
        <Button on:pointerenter=move |_| set_hover.set(true) on:pointerleave=move |_| set_hover.set(false) class=format!("justify-center items-end py-5 w-[3.75rem] h-full rounded-full {}", class).to_string() style:background-color=move || if hover.get() { tone(&color, contrast + 0.1, &Tone::Dark) } else { tone(&color.clone(), contrast, &Tone::Dark) }>
